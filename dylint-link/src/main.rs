...
fn main() -> Result<()> {
    ...
    let rustup_toolchain = env::var(env::RUSTUP_TOOLCHAIN)
        .unwrap_or_else(|_| "nightly-2024-07-19-x86_64-unknown-linux-gnu".to_owned());
    println!("Rustup toolchain: {}", rustup_toolchain);
...
fn copy_library(path: &Path) -> Result<()> {
    if_chain! {
        ...
        then {
            let rustup_toolchain = env::var(env::RUSTUP_TOOLCHAIN).unwrap_or_else(|_| "nightly-2024-07-19-x86_64-unknown-linux-gnu".to_owned());
